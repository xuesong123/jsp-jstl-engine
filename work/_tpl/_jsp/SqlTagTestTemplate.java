/*
 * $RCSfile: SqlTagTestTemplate.java,v $$
 * $Revision: 1.1 $
 * $Date: 2016-01-12 $
 *
 * Copyright (C) 2008 Skin, Inc. All rights reserved.
 *
 * This software is the proprietary information of Skin, Inc.
 * Use is subject to license terms.
 *
 * home: E:/WorkSpace/ayada/webapp
 * path: /sqlTagTest.jsp
 * lastModified: 2014-03-31 17:18:38 000
 * options: -fastJstl true
 * JSP generated by JspCompiler-1.0.1.2 (built 2016-01-12 23:30:07 067)
 */
package _tpl._jsp;

import java.io.Writer;
import com.skin.ayada.runtime.ExpressionContext;
import com.skin.ayada.runtime.JspWriter;
import com.skin.ayada.runtime.PageContext;
import com.skin.ayada.tagext.BodyContent;
import com.skin.ayada.tagext.BodyTag;
import com.skin.ayada.tagext.IterationTag;
import com.skin.ayada.tagext.JspFragment;
import com.skin.ayada.tagext.Tag;
import com.skin.ayada.template.JspTemplate;
import com.skin.ayada.util.ExpressionUtil;


/**
 * <p>Title: SqlTagTestTemplate</p>
 * <p>Description: </p>
 * <p>Copyright: Copyright (c) 2006</p>
 * @author JspCompiler
 * @version 1.0
 */
@SuppressWarnings("unused")
public class SqlTagTestTemplate extends JspTemplate {
    public static void main(String[] args){
        java.io.StringWriter writer = new java.io.StringWriter();
        PageContext pageContext = com.skin.ayada.runtime.JspFactory.getPageContext(writer);
        SqlTagTestTemplate template = new SqlTagTestTemplate();

        try {
            template.execute(pageContext);
            System.out.println(writer.toString());
        }
        catch(Throwable throwable) {
            throwable.printStackTrace();
        }
    }

    /**
     * @param pageContext
     * @throws Throwable
     */
    @Override
    @SuppressWarnings("unchecked")
    public void _execute(final PageContext pageContext) throws Throwable {
        JspWriter out = pageContext.getOut();
        JspWriter jspWriter = pageContext.getOut();
        ExpressionContext expressionContext = pageContext.getExpressionContext();
        // jsp:directive.page: lineNumber: 1
        // <jsp:directive.page contentType="text/html;charset=UTF-8"/>

        // NODE START: lineNumber: 2, offset: 2, length: 2, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_3
        // <sql:execute sql=\"delete from my_test2;\" out=\"${pageContext.getOut()}\"/>
        com.skin.ayada.jstl.sql.SqlTag _jsp_SqlTag_3 = new com.skin.ayada.jstl.sql.SqlTag();
        _jsp_SqlTag_3.setPageContext(pageContext);
        _jsp_SqlTag_3.setParent((Tag)null);
        _jsp_SqlTag_3.setSql("delete from my_test2;");
        _jsp_SqlTag_3.setOut(ExpressionUtil.evaluate(expressionContext, "${pageContext.getOut()}", java.lang.Object.class));
        int _jsp_start_flag_3 = _jsp_SqlTag_3.doStartTag();

        if(_jsp_start_flag_3 == Tag.SKIP_PAGE) {
            return;
        }
        if(_jsp_start_flag_3 != Tag.SKIP_BODY) {
            _jsp_SqlTag_3.doAfterBody();
        }
        _jsp_SqlTag_3.doEndTag();
        _jsp_SqlTag_3.release();
        // NODE END: lineNumber: 2, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_3

        // NODE START: lineNumber: 3, offset: 4, length: 2, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_5
        // <sql:execute sql=\"insert into my_test2(my_id, my_code, my_name) values (1, &#39;1&#39;, &#39;1&#39;);\"/>
        com.skin.ayada.jstl.sql.SqlTag _jsp_SqlTag_5 = new com.skin.ayada.jstl.sql.SqlTag();
        _jsp_SqlTag_5.setPageContext(pageContext);
        _jsp_SqlTag_5.setParent((Tag)null);
        _jsp_SqlTag_5.setSql("insert into my_test2(my_id, my_code, my_name) values (1, \'1\', \'1\');");
        int _jsp_start_flag_5 = _jsp_SqlTag_5.doStartTag();

        if(_jsp_start_flag_5 == Tag.SKIP_PAGE) {
            return;
        }
        if(_jsp_start_flag_5 != Tag.SKIP_BODY) {
            _jsp_SqlTag_5.doAfterBody();
        }
        _jsp_SqlTag_5.doEndTag();
        _jsp_SqlTag_5.release();
        // NODE END: lineNumber: 3, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_5

        // NODE START: lineNumber: 4, offset: 6, length: 2, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_7
        // <sql:execute home=\"${template.home}/database\" file=\"create.sql\" encoding=\"UTF-8\" out=\"${pageContext.getOut()}\"/>
        com.skin.ayada.jstl.sql.SqlTag _jsp_SqlTag_7 = new com.skin.ayada.jstl.sql.SqlTag();
        _jsp_SqlTag_7.setPageContext(pageContext);
        _jsp_SqlTag_7.setParent((Tag)null);
        _jsp_SqlTag_7.setHome(ExpressionUtil.getString(expressionContext, "${template.home}/database"));
        _jsp_SqlTag_7.setFile("create.sql");
        _jsp_SqlTag_7.setEncoding("UTF-8");
        _jsp_SqlTag_7.setOut(ExpressionUtil.evaluate(expressionContext, "${pageContext.getOut()}", java.lang.Object.class));
        int _jsp_start_flag_7 = _jsp_SqlTag_7.doStartTag();

        if(_jsp_start_flag_7 == Tag.SKIP_PAGE) {
            return;
        }
        if(_jsp_start_flag_7 != Tag.SKIP_BODY) {
            _jsp_SqlTag_7.doAfterBody();
        }
        _jsp_SqlTag_7.doEndTag();
        _jsp_SqlTag_7.release();
        // NODE END: lineNumber: 4, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_7

        // NODE START: lineNumber: 6, offset: 8, length: 20, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_9
        // <sql:execute>...</sql:execute>
        com.skin.ayada.jstl.sql.SqlTag _jsp_SqlTag_9 = new com.skin.ayada.jstl.sql.SqlTag();
        _jsp_SqlTag_9.setPageContext(pageContext);
        _jsp_SqlTag_9.setParent((Tag)null);
        int _jsp_start_flag_9 = _jsp_SqlTag_9.doStartTag();

        if(_jsp_start_flag_9 == Tag.SKIP_PAGE) {
            return;
        }
        if(_jsp_start_flag_9 != Tag.SKIP_BODY) {
            int _jsp_flag_9 = 0;
            if(_jsp_start_flag_9 == BodyTag.EVAL_BODY_BUFFERED) {
                BodyContent _jsp_body_content_9 = pageContext.pushBody();
                _jsp_SqlTag_9.setBodyContent(_jsp_body_content_9);
                _jsp_SqlTag_9.doInitBody();
                out = _jsp_body_content_9;
            }

            do {
                // NODE START: lineNumber: 7, offset: 9, length: 9, tagClassName: com.skin.ayada.jstl.core.ForEachTag, tagInstanceName: _jsp_ForEachTag_10
                // <c:forEach items=\"1, 2, 3, 4, 5\" var=\"id\">...</c:forEach>
                Object _jsp_old_var_10 = pageContext.getAttribute("id");
                com.skin.ayada.jstl.core.ForEachTag _jsp_ForEachTag_10 = new com.skin.ayada.jstl.core.ForEachTag();
                _jsp_ForEachTag_10.setParent(_jsp_SqlTag_9);
                _jsp_ForEachTag_10.setPageContext(pageContext);
                _jsp_ForEachTag_10.setVar("id");
                _jsp_ForEachTag_10.setItems("1, 2, 3, 4, 5");
                if(_jsp_ForEachTag_10.doStartTag() != Tag.SKIP_BODY) {
                    while(true) {
                        // TEXT: lineNumber: 7
                        // out.write("        insert into my_test1(my_id, my_code, my_name) values (");
                        out.write(_jsp_string_11, 0, _jsp_string_11.length);
                        // VARIABLE: lineNumber: 8
                        expressionContext.print(out, pageContext.getAttribute("id"));
                        // TEXT: lineNumber: 8
                        // out.write(", \'");
                        out.write(_jsp_string_13, 0, _jsp_string_13.length);
                        // VARIABLE: lineNumber: 8
                        expressionContext.print(out, pageContext.getAttribute("id"));
                        // TEXT: lineNumber: 8
                        // out.write("\', \'");
                        out.write(_jsp_string_15, 0, _jsp_string_15.length);
                        // VARIABLE: lineNumber: 8
                        expressionContext.print(out, pageContext.getAttribute("id"));
                        // TEXT: lineNumber: 8
                        // out.write("\');\r\n");
                        out.write(_jsp_string_17, 0, _jsp_string_17.length);
                        if(_jsp_ForEachTag_10.doAfterBody() != IterationTag.EVAL_BODY_AGAIN) {
                            break;
                        }
                    }
                }
                _jsp_ForEachTag_10.release();
                pageContext.setAttribute("id", _jsp_old_var_10);
                // jsp.jstl.core.ForEachTag END
                // NODE END: lineNumber: 7, tagClassName: com.skin.ayada.jstl.core.ForEachTag, tagInstanceName: _jsp_ForEachTag_10

                // NODE START: lineNumber: 10, offset: 18, length: 9, tagClassName: com.skin.ayada.jstl.core.ForEachTag, tagInstanceName: _jsp_ForEachTag_19
                // <c:forEach items=\"1, 2, 3, 4, 5\" var=\"id\">...</c:forEach>
                Object _jsp_old_var_19 = pageContext.getAttribute("id");
                com.skin.ayada.jstl.core.ForEachTag _jsp_ForEachTag_19 = new com.skin.ayada.jstl.core.ForEachTag();
                _jsp_ForEachTag_19.setParent(_jsp_SqlTag_9);
                _jsp_ForEachTag_19.setPageContext(pageContext);
                _jsp_ForEachTag_19.setVar("id");
                _jsp_ForEachTag_19.setItems("1, 2, 3, 4, 5");
                if(_jsp_ForEachTag_19.doStartTag() != Tag.SKIP_BODY) {
                    while(true) {
                        // TEXT: lineNumber: 10
                        // out.write("        insert into my_test2(my_id, my_code, my_name) values (");
                        out.write(_jsp_string_20, 0, _jsp_string_20.length);
                        // VARIABLE: lineNumber: 11
                        expressionContext.print(out, pageContext.getAttribute("id"));
                        // TEXT: lineNumber: 11
                        // out.write(", \'");
                        out.write(_jsp_string_22, 0, _jsp_string_22.length);
                        // VARIABLE: lineNumber: 11
                        expressionContext.print(out, pageContext.getAttribute("id"));
                        // TEXT: lineNumber: 11
                        // out.write("\', \'");
                        out.write(_jsp_string_24, 0, _jsp_string_24.length);
                        // VARIABLE: lineNumber: 11
                        expressionContext.print(out, pageContext.getAttribute("id"));
                        // TEXT: lineNumber: 11
                        // out.write("\');\r\n");
                        out.write(_jsp_string_26, 0, _jsp_string_26.length);
                        if(_jsp_ForEachTag_19.doAfterBody() != IterationTag.EVAL_BODY_AGAIN) {
                            break;
                        }
                    }
                }
                _jsp_ForEachTag_19.release();
                pageContext.setAttribute("id", _jsp_old_var_19);
                // jsp.jstl.core.ForEachTag END
                // NODE END: lineNumber: 10, tagClassName: com.skin.ayada.jstl.core.ForEachTag, tagInstanceName: _jsp_ForEachTag_19

                _jsp_flag_9 = _jsp_SqlTag_9.doAfterBody();
            }
            while(_jsp_flag_9 == IterationTag.EVAL_BODY_AGAIN);
            if(_jsp_start_flag_9 == BodyTag.EVAL_BODY_BUFFERED) {
                out = pageContext.popBody();
            }
        }
        _jsp_SqlTag_9.doEndTag();
        _jsp_SqlTag_9.release();
        // NODE END: lineNumber: 6, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_9

        // NODE START: lineNumber: 15, offset: 28, length: 3, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_29
        // <sql:execute>...</sql:execute>
        com.skin.ayada.jstl.sql.SqlTag _jsp_SqlTag_29 = new com.skin.ayada.jstl.sql.SqlTag();
        _jsp_SqlTag_29.setPageContext(pageContext);
        _jsp_SqlTag_29.setParent((Tag)null);
        int _jsp_start_flag_29 = _jsp_SqlTag_29.doStartTag();

        if(_jsp_start_flag_29 == Tag.SKIP_PAGE) {
            return;
        }
        if(_jsp_start_flag_29 != Tag.SKIP_BODY) {
            int _jsp_flag_29 = 0;
            if(_jsp_start_flag_29 == BodyTag.EVAL_BODY_BUFFERED) {
                BodyContent _jsp_body_content_29 = pageContext.pushBody();
                _jsp_SqlTag_29.setBodyContent(_jsp_body_content_29);
                _jsp_SqlTag_29.doInitBody();
                out = _jsp_body_content_29;
            }

            do {
                // TEXT: lineNumber: 15
                // out.write("delete from my_test2;\r\ninsert into my_test2(my_id, my_code, my_name) values (1001, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1002, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1003, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1004, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1005, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1006, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1007, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1008, \'1001\', \'1001\');\r\n");
                out.write(_jsp_string_30, 0, _jsp_string_30.length);
                _jsp_flag_29 = _jsp_SqlTag_29.doAfterBody();
            }
            while(_jsp_flag_29 == IterationTag.EVAL_BODY_AGAIN);
            if(_jsp_start_flag_29 == BodyTag.EVAL_BODY_BUFFERED) {
                out = pageContext.popBody();
            }
        }
        _jsp_SqlTag_29.doEndTag();
        _jsp_SqlTag_29.release();
        // NODE END: lineNumber: 15, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_29

        // NODE START: lineNumber: 27, offset: 31, length: 2, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_32
        // <sql:execute connection=\"${connection}\" sql=\"insert into my_test2(my_id, my_code, my_name) values (1009, &#39;1001&#39;, &#39;1001&#39;);\"/>
        com.skin.ayada.jstl.sql.SqlTag _jsp_SqlTag_32 = new com.skin.ayada.jstl.sql.SqlTag();
        _jsp_SqlTag_32.setPageContext(pageContext);
        _jsp_SqlTag_32.setParent((Tag)null);
        _jsp_SqlTag_32.setConnection(pageContext.getValue("connection", java.sql.Connection.class));
        _jsp_SqlTag_32.setSql("insert into my_test2(my_id, my_code, my_name) values (1009, \'1001\', \'1001\');");
        int _jsp_start_flag_32 = _jsp_SqlTag_32.doStartTag();

        if(_jsp_start_flag_32 == Tag.SKIP_PAGE) {
            return;
        }
        if(_jsp_start_flag_32 != Tag.SKIP_BODY) {
            _jsp_SqlTag_32.doAfterBody();
        }
        _jsp_SqlTag_32.doEndTag();
        _jsp_SqlTag_32.release();
        // NODE END: lineNumber: 27, tagClassName: com.skin.ayada.jstl.sql.SqlTag, tagInstanceName: _jsp_SqlTag_32

        // TEXT: lineNumber: 27
        // out.write("<table>\r\n    <tr>\r\n        <td>id</td>\r\n        <td>userName</td>\r\n        <td>nickName</td>\r\n    </tr>\r\n");
        out.write(_jsp_string_34, 0, _jsp_string_34.length);

        // NODE START: lineNumber: 35, offset: 34, length: 9, tagClassName: com.skin.ayada.jstl.sql.QueryTag, tagInstanceName: _jsp_QueryTag_35
        // <sql:query var=\"resultSet\" sql=\"select * from my_test2 where my_id &gt; 1001\" offset=\"2\">...</sql:query>
        com.skin.ayada.jstl.sql.QueryTag _jsp_QueryTag_35 = new com.skin.ayada.jstl.sql.QueryTag();
        try {
            _jsp_QueryTag_35.setPageContext(pageContext);
            _jsp_QueryTag_35.setParent((Tag)null);
            _jsp_QueryTag_35.setVar("resultSet");
            _jsp_QueryTag_35.setSql("select * from my_test2 where my_id > 1001");
            _jsp_QueryTag_35.setOffset(2);
            int _jsp_start_flag_35 = _jsp_QueryTag_35.doStartTag();

            if(_jsp_start_flag_35 == Tag.SKIP_PAGE) {
                return;
            }
            if(_jsp_start_flag_35 != Tag.SKIP_BODY) {
                int _jsp_flag_35 = 0;

                do {
                    // TEXT: lineNumber: 35
                    // out.write("    <tr>\r\n        <td>");
                    out.write(_jsp_string_36, 0, _jsp_string_36.length);
                    // EXPRESSION: lineNumber: 37
                    expressionContext.print(out, expressionContext.getString("resultSet.getLong(\"my_id\")"));
                    // TEXT: lineNumber: 37
                    // out.write("</td>\r\n        <td>");
                    out.write(_jsp_string_38, 0, _jsp_string_38.length);
                    // EXPRESSION: lineNumber: 38
                    expressionContext.print(out, expressionContext.getString("resultSet.getString(\"my_code\")"));
                    // TEXT: lineNumber: 38
                    // out.write("</td>\r\n        <td>");
                    out.write(_jsp_string_40, 0, _jsp_string_40.length);
                    // EXPRESSION: lineNumber: 39
                    expressionContext.print(out, expressionContext.getString("resultSet.getString(\"my_name\")"));
                    // TEXT: lineNumber: 39
                    // out.write("</td>\r\n    </tr>\r\n");
                    out.write(_jsp_string_42, 0, _jsp_string_42.length);
                    _jsp_flag_35 = _jsp_QueryTag_35.doAfterBody();
                }
                while(_jsp_flag_35 == IterationTag.EVAL_BODY_AGAIN);
            }
            _jsp_QueryTag_35.doEndTag();
            _jsp_QueryTag_35.release();
        }
        catch(Throwable throwable) {
            try {
                _jsp_QueryTag_35.doCatch(throwable);
            } catch (Throwable t) {
                throw new Exception(t);
            }
        }
        finally {
            _jsp_QueryTag_35.doFinally();
        }
        // NODE END: lineNumber: 35, tagClassName: com.skin.ayada.jstl.sql.QueryTag, tagInstanceName: _jsp_QueryTag_35

        // TEXT: lineNumber: 41
        // out.write("</table>\r\n");
        out.write(_jsp_string_44, 0, _jsp_string_44.length);

        out.flush();
        jspWriter.flush();
    }

    public static final char[] _jsp_string_11 = "        insert into my_test1(my_id, my_code, my_name) values (".toCharArray();
    public static final char[] _jsp_string_13 = ", \'".toCharArray();
    public static final char[] _jsp_string_15 = "\', \'".toCharArray();
    public static final char[] _jsp_string_17 = "\');\r\n".toCharArray();
    public static final char[] _jsp_string_20 = "        insert into my_test2(my_id, my_code, my_name) values (".toCharArray();
    public static final char[] _jsp_string_22 = ", \'".toCharArray();
    public static final char[] _jsp_string_24 = "\', \'".toCharArray();
    public static final char[] _jsp_string_26 = "\');\r\n".toCharArray();
    public static final char[] _jsp_string_30 = "delete from my_test2;\r\ninsert into my_test2(my_id, my_code, my_name) values (1001, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1002, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1003, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1004, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1005, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1006, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1007, \'1001\', \'1001\');\r\ninsert into my_test2(my_id, my_code, my_name) values (1008, \'1001\', \'1001\');\r\n".toCharArray();
    public static final char[] _jsp_string_34 = "<table>\r\n    <tr>\r\n        <td>id</td>\r\n        <td>userName</td>\r\n        <td>nickName</td>\r\n    </tr>\r\n".toCharArray();
    public static final char[] _jsp_string_36 = "    <tr>\r\n        <td>".toCharArray();
    public static final char[] _jsp_string_38 = "</td>\r\n        <td>".toCharArray();
    public static final char[] _jsp_string_40 = "</td>\r\n        <td>".toCharArray();
    public static final char[] _jsp_string_42 = "</td>\r\n    </tr>\r\n".toCharArray();
    public static final char[] _jsp_string_44 = "</table>\r\n".toCharArray();

}
